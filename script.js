...
document.querySelectorAll('.feature__item').forEach(card => {
  const iconObject = card.querySelector('.feature__icon object');

  if (!iconObject) {
    console.warn('⚠️ Не найден object в карточке:', card);
    return;
  }

  card.addEventListener('mouseenter', () => {
    iconObject.style.opacity = 0.5; // Плавно уменьшаем прозрачность
    setTimeout(() => {
      iconObject.data = card.dataset.hover;
      iconObject.style.opacity = 1;
    }, 150); // Ждём полупрозрачности перед заменой
  });

  card.addEventListener('mouseleave', () => {
    iconObject.style.opacity = 1.5;
    setTimeout(() => {
      iconObject.data = card.dataset.normal;
      iconObject.style.opacity = 1.5;
    }, 150);
  });
});